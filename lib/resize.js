...
var imageminPngquant = require('imagemin-pngquant-chn');
    ...
    if (err1) {
      ...
      if (stats.isDirectory()) {
        ...
        fs.exists(imgPath, function (exists) {
          if (!exists) {
            images(path).resize(300).save(imgPath, {
              quality: 100,
            });
          }
        });
        ...
        fs.exists(imgLargePath, function (exists) {
          if (!exists) {
            images(path).resize(621).save(imgLargePath);
          }
        });
...
module.exports = {
  init: function (path, floor) {
    ...
    Promise.all([tmpPromise, tmpWebPPromise, tmpLargePromise]).then(
      function () {
        walk(path, floor);
        minifiy();
      }
